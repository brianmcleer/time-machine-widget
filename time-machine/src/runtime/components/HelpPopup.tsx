...
export interface HelpSection {
  /** Stable key: search behavior and the opening section depend on it. Keep the
   *  standard keys (start, save, load, menu, share, organize, keep, trouble,
   *  tips) so search behaves the same way across widgets. */
  key: string
  /** Calcite icon name, e.g. 'play', 'save', 'map', 'ellipsis', 'share',
   *  'search', 'folder', 'exclamation-mark-triangle', 'lightbulb'. */
  icon: string
  title: string
...
export interface HelpPopupProps {
  ...
  initialKey?: string
  /* Strings are passed in rather than read from a translate hook, so this
   * component works the same whether the parent widget is a function
   * component with useIntl or a class component reading translations
   * directly. Nothing else about it changes per widget. */
  title: string
  intro: string
...
/**
 * In-widget help written for people who do not enjoy technology: plain words, one idea per
 * line, the same button names the UI uses, and a search box so nobody has to read it all.
 * Content is filtered to the features this app has turned on and lives in translations so it
 * can be localized with the rest of the UI.
 *
 * Presentation only. The content for a widget lives in its own helpSections file, and this
 * file is meant to be identical across every widget so the guide looks and behaves the same
